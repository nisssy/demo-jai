...
const SAMPLE_IMAGES = [
  { id: "s1", label: "サンプルA", color: "from-blue-200 to-indigo-300" },
  { id: "s2", label: "サンプルB", color: "from-pink-200 to-rose-300" },
  { id: "s3", label: "サンプルC", color: "from-emerald-200 to-teal-300" },
]

export const PosterOrderModal = ({ open, onOpenChange, vendorId, onVendorIdChange, onSubmit, eventName }: Props) => {
  ...
  const [subject, setSubject] = useState(`【ポスター発注】${eventName ?? "抽選会"}のポスター印刷について`)
  const [body, setBody] = useState(
    `お世話になっております。\n下記の通りポスター発注をお願いいたします。\n\n・数量: 1000枚\n・初稿希望日: \n・最終納品日: \n\nよろしくお願いいたします。`,
  )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[60vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>ポスター発注</DialogTitle>
          <DialogDescription>内容を入力して印刷会社へ送付します</DialogDescription>
        </DialogHeader>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-xs">発注先（印刷会社）</Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <Label className="text-xs">最終納品日</Label>
                <Input type="date" value={finalDate} onChange={(e) => setFinalDate(e.target.value)} className="h-9 text-xs" />
              </div>
            </div>

            <div className="space-y-2">
              <Label className="text-xs">ポスター枚数</Label>
          ...
          <div className="space-y-2">
            ...
            <div className="border rounded-lg p-5 bg-white text-xs space-y-3 min-h-[480px]">
              ...
              <div>
                ...
                <div className="font-medium">{printingPartners.find((p) => p.id === vendorId)?.name ?? "-"}</div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <div className="text-slate-500 text-[11px]">最終納品日</div>
                  <div>{finalDate || "-"}</div>
                </div>
              </div>
              <div>
        ...
        <div className="flex justify-end gap-2 border-t pt-3">
          ...
          <Button size="sm" onClick={onSubmit}>発注して送付</Button>
        </div>
      </DialogContent>
    </Dialog>
  )
