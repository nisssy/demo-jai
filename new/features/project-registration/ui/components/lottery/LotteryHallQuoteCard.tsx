...
type LotteryHallQuoteCardProps = {
  hallQuote: HallQuote
  dmMailing: "yes" | "no"
  onPdfExport: () => void
...
export const LotteryHallQuoteCard = ({
  hallQuote,
  dmMailing,
  onPdfExport,
  onNotifyCustomer,
}: LotteryHallQuoteCardProps) => {
        ...
        <table className="w-full text-xs">
            ...
            {hallQuote.quoteItems
              ...
              .map((item) => (
                <tr key={item.id} className="border-t border-slate-50">
                  ...
                  <td className="py-1 text-right text-slate-600">{item.id === 2 ? item.quantity : "-"}</td>
                  <td className="py-1 text-right text-slate-600">¥{item.unitPrice.toLocaleString()}</td>
          ...
          <tfoot>
            <tr className="border-t border-slate-200">
              ...
              <td className="py-1 text-right font-semibold text-slate-900">¥{itemTotal.toLocaleString()}</td>
            </tr>
          </tfoot>
        </table>
      </div>
