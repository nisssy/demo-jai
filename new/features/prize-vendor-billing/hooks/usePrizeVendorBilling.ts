...
import type { MonthlyBilling, BillingChatMessage } from "@/new/api/types"
import { SEED_PRIZE_VENDORS } from "@/new/api/seed-data"

export function usePrizeVendorBilling(repository: ProjectRepository) {
  const [selectedVendorId, setSelectedVendorId] = useState<string>(SEED_PRIZE_VENDORS[0]?.id ?? "1")
  const [selectedBillingId, setSelectedBillingId] = useState<string | null>(null)
  const [refreshKey, setRefreshKey] = useState(0)

  const vendors = useMemo(() => SEED_PRIZE_VENDORS, [])

  const billings = useMemo(() => {
    return repository.getMonthlyBillingsByVendor(selectedVendorId).filter(
      (b) => b.vendorType === "prize"
    )
  ...
  const requestCorrection = useCallback(
    (billingId: string) => {
      ...
      const chatMessage: BillingChatMessage = {
        author: selectedVendorName || "景品業者",
