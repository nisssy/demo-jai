...
export type SurveyResultModalViewProps = {
  ...
  clientName: string
  onDownloadCsv: (product: Product) => void
}

export function SurveyResultModalView({
  open,
  onOpenChange,
  product,
  clientName,
  onDownloadCsv,
}: SurveyResultModalViewProps) {
  const surveyResult = product?.surveyResult
  const hasSurveyResult = surveyResult &&
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {hasSurveyResult && (
                  <Button variant="outline" size="sm" onClick={() => onDownloadCsv(product)} className="gap-2">
              ...
              {hasSurveyResult ? (
                ...
                <div className="border rounded-lg p-4 text-center text-slate-500">
                  ...
                  <p className="text-xs mt-2">送付日: {product.surveySentDate || "未送付"}</p>
                </div>
              )}
            </div>
          </div>
