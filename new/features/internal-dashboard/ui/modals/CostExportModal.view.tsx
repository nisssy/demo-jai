...
export function CostExportModalView({
  open,
  onOpenChange,
  dateFrom,
  dateTo,
            ...
            <div className="space-y-4">
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-semibold text-slate-900">{targetProducts.length}件</span>
                </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {targetProducts.map((product) => {
                        const productTotal =
                          (product.castingCost || 0) +
                          (product.transportationFee || 0) +
                          (product.accommodationFee || 0) +
                          (product.postPRCost || 0)
                        return (
                          <TableRow key={product.id}>
                            <TableCell className="font-medium">{product.eventProductName}</TableCell>
                            <TableCell>{product.projectNumber}</TableCell>
                            <TableCell>{product.eventDate || "未定"}</TableCell>
                            <TableCell className="text-right">¥{Math.round(productTotal).toLocaleString()}</TableCell>
                          </TableRow>
                        )
                      })}
                    </TableBody>
