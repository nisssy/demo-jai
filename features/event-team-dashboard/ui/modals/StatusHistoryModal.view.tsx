...
type StatusHistoryEntry = { status: string; timestamp: string; changedBy?: string; note?: string }
...
export const StatusHistoryModalView = ({ open, onOpenChange, project }: StatusHistoryModalViewProps) => {
  const history = (project as any)?.statusHistory as StatusHistoryEntry[] | undefined
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>{project?.projectName} のステータス変更履歴</DialogDescription>
        </DialogHeader>
          ...
          {project && (
            <div className="space-y-2">
              ...
              <div className="text-sm text-slate-600 space-y-1">
                <div>案件No: {project.projectNumber}</div>
                <div>クライアント: {project.clientName}</div>
                <div>実施日: {project.date}</div>
              </div>
            </div>
          )}
          <div className="space-y-2">
