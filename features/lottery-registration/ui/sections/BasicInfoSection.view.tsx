...
import type { HallData, CompanyData, EmployeeData } from "@/lib/demo-db/types"
...
import { HallRowView } from "./HallRow.view"
import { useState } from "react"

export type BasicInfoSectionViewProps = {
...
export function BasicInfoSectionView({
  halls,
  dmMailing,
  eventStartDate,
  eventEndDate,
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => onEventStartDateChange(e.target.value)}
                  ...
                  <CommandGroup>
                    {filteredSalesEmployees.slice(0, 15).map((e) => (
                      <CommandItem
                        ...
                        value={e.name}
                        onSelect={() => {
                          onSalesPersonChange(String(e.id), e.name)
                          setSalesOpen(false)
                          setSalesQuery("")
                        }}
                        className="text-xs"
                  ...
                  <CommandGroup>
                    {filteredInsightEmployees.slice(0, 15).map((e) => (
                      <CommandItem
                        ...
                        value={e.name}
                        onSelect={() => {
                          onInsightPersonChange(String(e.id), e.name)
